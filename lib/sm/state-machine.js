 ...
 * @param {!Array.<sm.TransitionDefinition>} transitions
 * @param {!sm.StateMachine=} opt_propagationParent
 */
sm.StateMachine = function(transitions, opt_propagationParent) {
  events.EventEmitter.call(this, opt_propagationParent);

  /**
   * @type {sm.TransitionFrom}
   */
 ...
 */
sm.StateMachine.prototype.__transform = function(transitions) {
  var result = {};
  ...
  while (i < l) {
    ...
    if (typeof result[fromEvent][toEvent] === 'undefined') {
      result[fromEvent][toEvent] = sm.createTransition(check, script);
    }
